...
1. Grant the `get` and `list` secrets permissions in the Azure Key Vault instance to each Spring Apps application's managed identity by using Azure Key Vault access policy:
   ...
   ```bash
   CUSTOMERS_SERVICE_UID=$(az identity show -g $RESOURCE_GROUP -n uid-customers-svc-$UNIQUEID --query principalId -o tsv)
   VISITS_SERVICE_UID=$(az identity show -g $RESOURCE_GROUP -n uid-visits-svc-$UNIQUEID --query principalId -o tsv)
   VETS_SERVICE_UID=$(az identity show -g $RESOURCE_GROUP -n uid-vets-svc-$UNIQUEID --query principalId -o tsv)
