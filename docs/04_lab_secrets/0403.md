...
1. Create and assign an identity to each of the three apps by running the following commands from Git Bash shell:
   ...
   ```bash
   CUSTOMERS_SERVICE_ID=$(az identity create -g $RESOURCE_GROUP -n uid-customers-svc-$UNIQUEID --query id -o tsv)
    ...
    VISITS_SERVICE_ID=$(az identity create -g $RESOURCE_GROUP -n uid-visits-svc-$UNIQUEID --query id -o tsv)
    ...
    VETS_SERVICE_ID=$(az identity create -g $RESOURCE_GROUP -n uid-vets-svc-$UNIQUEID --query id -o tsv)
